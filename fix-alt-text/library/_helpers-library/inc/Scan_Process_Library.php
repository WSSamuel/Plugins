		...
		if ( file_exists( $file_path ) ) {
			...
			if ( $fh === false ) {
				...
				foreach ( $group as $line ) {
					$current_line = trim( stream_get_line( $fh, 10240, self::get_end_marker() ) );
	...
	public function get_group( string $portion ): array {
		...
		if ( file_exists( $file_path ) ) {
			...
			if ( $handle === false ) {
				...
				$current_line = trim( stream_get_line( $handle, 10240, self::get_end_marker() ) );
				...
				if ( $current_line . self::get_end_marker() == $flag ) {
					...
					while ( $num <= $group_size && $current_line ) {
						...
						if ( $portion === trim( $line[1] ) ) {

							$group[] = trim($current_line);
