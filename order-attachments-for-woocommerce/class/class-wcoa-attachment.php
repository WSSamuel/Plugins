...
class WCOA_Attachment
{
	...
	private function upload()
    {
		...
		//add_post_meta($attachment_id, '_wcoa_order_id', $this->order_id, true);

		add_post_meta($this->order_id, '_wcoa_attachment_id', $attachment_id, false);
	...
	public static function prepare_customer_list(int $user_id): array
	{
		...
		$sql_check = "SELECT COUNT(*) FROM $wpdb->posts AS attachment
                      JOIN $wpdb->postmeta AS attachment_meta ON (attachment_meta.meta_value = attachment.ID)
                      JOIN $wpdb->postmeta AS orders ON attachment_meta.post_id = orders.post_id
                      WHERE attachment.post_type = 'attachment' AND attachment_meta.meta_key = '_wcoa_attachment_id'
                      AND orders.meta_key = '_customer_user' AND attachment.post_mime_type != '' AND orders.meta_value = $user_id;";
		...
		$sql = "SELECT orders.post_id AS 'order_id', attachment.post_date, attachment.guid FROM $wpdb->posts AS attachment
                JOIN $wpdb->postmeta AS attachment_meta ON (attachment_meta.meta_value = attachment.ID)
                JOIN $wpdb->postmeta AS orders ON attachment_meta.post_id = orders.post_id
                WHERE attachment.post_type = 'attachment' AND attachment_meta.meta_key = '_wcoa_attachment_id'
                AND orders.meta_key = '_customer_user' AND attachment.post_mime_type != '' AND orders.meta_value = $user_id;";
	...
	public static function get_url(int $order_id, string $order_key, int $attachment_id = 0)
    {
		global $wpdb;
		...
		if ($attachment_id === 0)
        {
            $attachment_id = $wpdb->get_var("SELECT MIN(meta_value) AS qty FROM $wpdb->postmeta where post_id = $order_id and meta_key = '_wcoa_attachment_id'");
        }
