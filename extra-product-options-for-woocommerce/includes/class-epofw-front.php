...
if ( ! class_exists( 'EPOFW_Front' ) ) {
	...
	class EPOFW_Front {
		...
		public function epofw_add_cart_item_data( $cart_item, $product_id ) {
			...
			$epofw_nonce = filter_input( INPUT_POST, '_wpnonce', FILTER_SANITIZE_FULL_SPECIAL_CHARS );
			if ( empty( $epofw_nonce ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $epofw_nonce ) ), 'epfow_addon_field_nonce_' . $product_id ) ) {
				$message = apply_filters( 'epfow_addon_field_nonce_error_to_validate_cart_item', __( 'Nonce verification failed', 'extra-product-options-for-woocommerce' ), $product_id );
