...
/***  Copyright 2012 - 2021  Kiboko Labs
...
include_once(BFT_PATH."/controllers/webhooks.php");
...
function bft_init() {
   ...
   define( 'BFT_WEBHOOKS', $wpdb->prefix."bft_webhooks");
   ...
   add_action('wp_enqueue_scripts', 'arigato_scripts');
	...
	// WPForms integration
    add_action( 'wpforms_process_complete', array('BFTIntegrations', 'signup_wpforms'), 30, 4 );
	...
	// Zapier / webhooks		
	add_action('arigato_subscribed', ['ArigatoWebhooks', 'dispatch_subscribe'], 10, 2);
	add_action('arigato_confirmed', ['ArigatoWebhooks', 'dispatch_confirmed']);
	add_action('arigato_unsubscribed', ['ArigatoWebhooks', 'dispatch_unsubscribe']);
	...
	if($wpdb->get_var("SHOW TABLES LIKE '".BFT_EMAILLOG."'") == BFT_EMAILLOG) {
		...
		$wpdb->query($wpdb->prepare("DELETE FROM ".BFT_EMAILLOG." WHERE date < CURDATE() - INTERVAL %d DAY", intval($cleanup_raw_log)));				
	}
	
	define('BFT_SLEEP', floatval(get_option('bft_sleep')));
	
	$version = get_option('bft_db_version');
	if(empty($version) or version_compare($version, '2.24') == -1) bft_install(true);
...
function arigato_scripts() {
	wp_register_style( 'arigato-css', BFT_URL . 'front.css', array(), '2.6.4');
...
function bft_autoresponder_menu() {  
  ...
  add_submenu_page("bft_options",__('Webhooks / Zapier', 'broadfast'), __('Webhooks / Zapier', 'broadfast'), $arigato_caps, "bft_webhooks", array("ArigatoWebhooks", "manage"));
  ...
  if(defined('WPFORMS_VERSION')) add_submenu_page('bft_options',__('Integrate in WPForms', 'broadfast'), __('Integrate in WPForms', 'broadfast'), $arigato_caps, "bft_integrate_wpf", array("BFTIntegrations", "wpforms"));
...
function bft_install($update = false) {
    ...
    require_once( ABSPATH . 'wp-admin/includes/upgrade.php' );
	 ...
	 $sql = "CREATE TABLE " . BFT_USERS . " (
	  ...
	  // add indexes
	  if(empty($old_bft_db_version) or $old_bft_db_version < 2.24 ) {
        $sql = "CREATE INDEX email ON ".BFT_USERS." (email) ";
        $wpdb->query($sql);
        
        $sql = "CREATE INDEX days ON ".BFT_MAILS." (days) ";
        $wpdb->query($sql);
        
        $sql = "CREATE INDEX user_id ON ".BFT_SENTMAILS." (user_id) ";
        $wpdb->query($sql);        
        
        $sql = "CREATE INDEX user_mail_date ON ".BFT_SENTMAILS." (user_id, date, mail_id) ";
        $wpdb->query($sql);        
	  }
      ...
      // and when empty, we'll display form
		...
		if(!empty($unsub_post_id) and empty($_POST['bft_unsubscribe']) and !strstr(@$post->content, '[bft-unsubscribe]')) {
	  	...
	  	if(empty($unsub_post_id) or !empty($_POST['bft_unsubscribe'])) {
	  		...
	  		//  notify admin?
			if(get_option('bft_unsubscribe_notify')) bft_unsubscribe_notify($user);
			do_action('arigato_unsubscribed', $user);
